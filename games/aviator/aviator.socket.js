...
const processCashout = async (ws, bet, cashoutBetId, clientMultiplier) => {
  ...
  try {
    ...
    sendBetError(
      ws,
      error.message || "Unable to cash out",
      "CashoutBet",
      bet.betType,
    );
      ...
      try {
        ...
        if (data.action === "PlaceBet") {
          ...
          if (gameState.status !== "WAIT") {
            sendBetError(
              ws,
              "Bets can only be placed during the waiting period",
              "PlaceBet",
              data.betType,
            );
            return;
        ...
        if (data.action === "CancelBet") {
          ...
          if (gameState.status !== "WAIT") {
            sendBetError(
              ws,
              "Bets can only be cancelled during the waiting period",
              "CancelBet",
              data.betType,
            );
            return;
          ...
          try {
            ...
            gameState.totalBetPool = Math.max(
              0,
              gameState.totalBetPool - bet.stake,
            );
            ...
            sendBetError(
              ws,
              error.message || "Unable to cancel bet",
              "CancelBet",
              data.betType,
            );
          }
        ...
        if (data.action === "CashoutBet") {
          ...
          if (!isCashoutAllowed()) {
            sendBetError(
              ws,
              "Cashout is only available while the round is running",
              "CashoutBet",
              data.betType,
            );
            return;
          ...
          if (bet.roundId && bet.roundId !== gameState.roundId) {
            sendBetError(
              ws,
              "Bet is not for the current round",
              "CashoutBet",
              data.betType,
            );
            return;
